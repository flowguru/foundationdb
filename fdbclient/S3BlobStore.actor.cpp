...
Reference<HTTP::OutgoingRequest> getDryrunRequest(Reference<S3BlobStoreEndpoint> bstore, std::string bucket) {
	...
	// next step is to try a non-exist bucket
		...
		try {
			...
			try {
				if (s3TokenError && isWriteRequest(req->verb)) {
					// retry with a GET, who is more lightweight than a write request
					...
					if (bucket.empty()) {
						TraceEvent(SevWarnAlways, "EmptyBucketRequest")
					...
					TraceEvent("Hfu5Resource")
					    .detail("Resource", dryrunRequest->resource)
					    .log();
				...
				TraceEvent(SevWarnAlways, "ErrorDuringRetryS3TokenIssue").errorUnsuppressed(e);
			}
