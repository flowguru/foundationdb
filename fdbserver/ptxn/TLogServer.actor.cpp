...
Key persistStorageTeamMessageRefsKey(UID id, StorageTeamID storageTeamId, Version version) {
	BinaryWriter wr(Unversioned());
	wr.serializeBytes(persistStorageTeamMessageRefsKeys.begin); // 18
	wr << id; // 16
	wr << storageTeamId; // 16
	wr << bigEndian64(version); // 8
	return wr.toValue();
}

Version decodeVersionFromStorageTeamMessageRefs(KeyRef key) {
	StringRef stripKey = key.substr(sizeof(UID) + sizeof(StorageTeamID) + persistStorageTeamMessageRefsKeys.begin.size());
		...
		if (logData->shouldSpillByValue(req.storageTeamID)) {
			...
			RangeResult kvrefs = wait(tlogGroup->second->persistentData->readRange(
			    KeyRangeRef(persistStorageTeamMessageRefsKey(logData->logId, req.storageTeamID, req.beginVersion),
			                persistStorageTeamMessageRefsKey(logData->logId, req.storageTeamID, logData->persistentDataDurableVersion + 1)),
			...
			for (const auto& pair : commitLocations) {
				messageReads.push_back(tlogGroup->second->rawPersistentQueue->read(pair.first, pair.second, CheckHashes::True));
			}
			...
			loop {
				...
				// locations stored in persistStorageTeamMessageRefsKey( is for the whole TLogQueueEntry, not a certain team
		...
		if (teamData) {
			...
			while (msg != teamData->versionMessages.end() && msg->first <= newPersistentDataVersion) {
				...
				if (logData->shouldSpillByValue(teamData->storageTeamId)) {
					...
					SpilledData spilledData(currentVersion, begin, length, size); // question: do we want to not use binary writer here anymore?
					wr << spilledData;
					...
					if ((wr.getLength() + sizeof(SpilledData) >
						...
						*(uint32_t*)wr.getData() = refSpilledTagCount; 
						// question: why we store a count in the first 32 bytes of a BinaryWriter, to indicate how many
						// entities are being written in this binary writer
						self->persistentData->set(KeyValueRef(
